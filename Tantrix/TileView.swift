...
class TileView: UIView {
    ...
    private func drawHexagon() {
        ...
        hexagon.move(to: CGPoint(x: center.x + bounds.width / 2 * sin(-30.0 * .pi / 180),
                                 y: center.y - bounds.width / 2 * cos(-30.0 * .pi / 180)))
    ...
    private func drawCurves() {
        let center = CGPoint(x: bounds.midX, y: bounds.midY)
        let sideRadius = bounds.width / 2 * cos(30.0 * .pi / 180)  // from center to midpoint of side
